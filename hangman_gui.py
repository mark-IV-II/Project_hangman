...
def playaudio(audiofile):
    ...
    audiofile_path = os.path.join(".","sfx",audiofile)
    playsound(audiofile_path)
...
def quit_game():
    score_line = 'Thank you for playing Hangman. Your score is ' + \
                 str(params.score)
    playaudio("end.wav")
    ...
    if len(char) == 1:
        ...
        if not char.isalpha():
            ...
            messagebox.showwarning(
                'Oops', 'You have entered an invalid charactor. Please try again')
            display_all_elements()
            return None
            ...
            if char not in word:
                ...
                if params.attempts == 0:
                    score_line = 'Oops, you have exhausted your attempts. Your score is ' + \
                                 str(params.score)
                    playaudio("end.wav")
        ...
        messagebox.showwarning(
            'Uh oh', 'Only one alphabet is allowed at a time. Please try again')
...
welcome_label = ttk.Label(root, text=welcome_line)
welcome_label.config(font=('verdana', 16))
attempt_label = ttk.Label(root, text='Attempts left')
score_label = ttk.Label(root, text='Score')
output_label = ttk.Label(root, text='The word to guess is : ')
output_label.config(font=('comic sans', 14))
input_label = ttk.Label(root, text='Enter your guess')
missed_label = ttk.Label(root, text='Misfires')
source_code_label = ttk.Label(
    root, text='Source code can be found here: https://github.com/mark-IV-II/Project_hangman.git \n All sound effects from https://www.noiseforfun.com')

attempt_box = ttk.Entry(root, width=5)
score_box = ttk.Entry(root, width=5)
output_box = ttk.Entry(
    root, font='Calibri 16 bold', width=25, justify='center')
input_box = ttk.Entry(root, width=5)
missed_box = ttk.Entry(root, width=30)
quit_button = ttk.Button(
    root,  width=10, text='Quit', command=quit_game)
check_button = ttk.Button(
    root,  width=15, text='Check', command=check_ans)
reset_button = ttk.Button(
    root,  width=10, text='Reset', command=reset)
...
game_canvas.create_window(250, 430, window=source_code_label)
